...
import { TSStore, type PetCreated as Pet } from "../../store/ts-store";
import { z } from "zod";
...
export const config: EventConfig = {
  type: "event",
  name: "PetLifecycleOrchestrator",
  description: "Pet lifecycle state management with staff interaction points",
  subscribes: [
    "lc.pet.created",
    "lc.feeding.reminder.completed",
    "lc.pet.status.update.requested",
  ],
  emits: [],
  flows: ["PetManagement"],
  input: z.object({
...
export const handler: Handlers["PetLifecycleOrchestrator"] = async (
  input,
  { emit, logger }
) => {
  ...
  try {
    const pet = await TSStore.get(petId);
    if (!pet) {
      if (logger) {
    ...
    const updatedPet = await TSStore.update(petId, { status: rule.to });
